        ...
        <v-data-table v-model="selected" :headers="headers" :items="users" single-select item-key="userId" show-select
            ...
            <template v-slot:top>
                <v-row>
                    ...
                    <v-col>
                        <v-btn v-if="activeProfile === 'Admin'" depressed :disabled="selected.length === 0" color="error"
                            @click="deleteUser">
                            Delete User
                        </v-btn>
                    </v-col>
        ...
        <v-dialog v-model="detailsDialog" max-width="600px">
            <v-card>
                <v-container>
                    ...
                    <v-card-text>{{ detailsUser.firstName }} {{ detailsUser.lastName }}</v-card-text>
                    <v-card-title>Email:</v-card-title>
                    <v-card-text>{{ detailsUser.emailAddress }}</v-card-text>
                    <v-card-title>Profile:</v-card-title>
                    <v-card-text>{{ detailsUser.userProfile }}</v-card-text>
...
export default {
    ...
    methods: {
        ...
        deleteUser() {
            this.messages = [];
            axios
                .delete('https://localhost:7060/api/Users/' + this.selected[0].userId)
                .then(() => {
                    this.messages.push('User deleted successfully')
                    axios
